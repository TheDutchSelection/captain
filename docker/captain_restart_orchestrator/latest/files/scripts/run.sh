...
set_restart_from_needs_restart_etcd_key () {
  ...
  local current_restart_value=$(get_etcd_value "$restart_key")
  if [[ "$current_restart_value" != "$restart_value" ]]; then
    set_etcd_value "$restart_key" "$restart_value"
  fi
}
...
while true; do
  need_restart_keys="$(get_etcd_keys_from_partial_key_with_value $ETCD_BASE_PATH $short_need_restart_key $need_restart_value)"
  ...
  while read -r need_restart_key; do
    if [[ ! -z "$need_restart_key" ]]; then
      ...
      relevant_etcd_app_path="$(get_etcd_app_path_from_app_key $need_restart_key)"
      ...
      current_restarts="$(count_etcd_keys_with_value_for_app $relevant_etcd_app_path $restart_key $restart_value)"
      ...
      if [[ "$max_restarts" > "$current_restarts" ]]; then
        ...
        current_update_value=$(get_etcd_value "$update_key")
